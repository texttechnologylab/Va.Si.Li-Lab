...
namespace VaSiLi.SceneManagement
{
    public class RoleMenuManager : MonoBehaviour
    {
        ...
        void UpdateRoles(Dictionary<string, ApiRole> roles)
        {
            int controlI = 0;

            if (roles == null || roles.Keys.Count == 0)
                return;

            ApiRole?[] pickedRoles = RoleManager.GetPickedRoles();

            foreach (ApiRole role in roles.Values)
            {
        ...
        private void UpdateHiddenAvatarWithSpectatorLevels(Dictionary<string, ApiRole> roles)
        {
            foreach (ApiRole r in roles.Values)
            {
                if (r.IsSpectator)
                {
                    //TODO: hiddenRole.level = r.level;
                }
            }
        ...
        {
            return roles.Count((item) => item.HasValue && item.Value.id == role.id) >= role.maxCount;
        }

        private async void UpdateAvailableRoles()
        {
            if (SceneManager.CurrentScene != null)
                await RoleManager.FetchRoles(SceneManager.CurrentScene.Value);
        }
